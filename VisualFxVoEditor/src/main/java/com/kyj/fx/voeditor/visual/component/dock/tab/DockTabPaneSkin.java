			...
			headersRegion.addEventHandler(MouseEvent.MOUSE_RELEASED, e -> {
				...
				if (MouseButton.PRIMARY == e.getButton() && e.getClickCount() == 1) {
					...
					if (!headersRegion.contains(e.getX(), e.getY()) /*&& isMovingTarget.get()*/) {
						...
						//						DockTab dockTab = getSkinnable().getTabs().get(selectIdx);
		...
		public TabHeaderSkin(final DockTab tab) {
			...
			inner = new StackPane() {
				...
				protected void layoutChildren() {
					...
					positionInArea(label, labelStartX, paddingTop, labelAreaWidth, h, /*baseline ignored*/0, HPos.CENTER, VPos.CENTER);

					if (closeBtn.isVisible()) {
						closeBtn.resize(closeBtnWidth, closeBtnHeight);
						positionInArea(closeBtn, closeBtnStartX, paddingTop, closeBtnWidth, h, /*baseline ignored*/0, HPos.CENTER,
								VPos.CENTER);
					}
			...
			inner = new StackPane() {
				...
				private void positionArrow(Pane btn, StackPane arrow, double x, double y, double width, double height) {
					...
					positionInArea(arrow, btn.snappedLeftInset(), btn.snappedTopInset(),
							...
							height - btn.snappedTopInset() - btn.snappedBottomInset(), /*baseline ignored*/0, HPos.CENTER, VPos.CENTER);
				}
			};
