 ...
 *
 */
public class CPagenationSkin extends BehaviorSkinBase<Pagination, PaginationBehavior> {
	...
	public void selectPreviousPage() {
		...
		int previousPageStart = (currentPageIndex / 10 - 1) * 10 ;
		if (previousPageStart >= 0) {
			pagination.setCurrentPageIndex(previousPageStart);
		...
		Platform.runLater(() -> {
			...
			if (currentAnimatedIndex > previousIndex) { // animate right to left
				...
				KeyFrame k1 = new KeyFrame(Duration.millis(0),
						new KeyValue(currentStackPane.translateXProperty(), useTranslateX ? currentStackPane.getTranslateX() : 0,
								interpolator),
				...
				KeyFrame k2 = new KeyFrame(DURATION, swipeAnimationEndEventHandler,
	...
	private void animateClamping(boolean rightToLeft) {
		if (rightToLeft) { // animate right to left
			...
			KeyFrame k2 = new KeyFrame(DURATION, clampAnimationEndEventHandler,
	...
	class NavigationControl extends StackPane {
		...
		public NavigationControl() {
			...
			//			rightNextPageArrowButton.prefHeightProperty().bind(rightNextPageArrowButton.minHeightProperty());
			//			rightNextPageArrowButton.prefWidth(40);
			leftNextPageArrowButton.getStyleClass().add("left-next-arrow-button");
		...
		private void initializeNavigationHandlers() {
			...
			leftNextPageArrowButton.setOnAction(arg0 -> {
				selectPreviousPage();
				requestLayout();
			});
		...
		private void initializePageIndicators() {
			...
			controlBox.getChildren().add(leftNextPageArrowButton);
		...
		private void layoutPageIndicators() {
			...
			final double leftNextPageArrowWidth = snapSize(Utils.boundedSize(leftNextPageArrowButton.prefWidth(-1),
					leftNextPageArrowButton.minWidth(-1), leftNextPageArrowButton.maxWidth(-1)));
			...
			double w = width - (leftNextPageArrowWidth + controlBoxleft + leftArrowWidth + 2 * arrowButtonGap.get() + spacing
					+ rightArrowWidth + rightNextPageArrowWidth + controlBoxRight);
		...
		private boolean changePageSet() {
			...
			if (previousIndex < currentIndex && index == 0 && lastIndicatorButtonIndex != 0 && index % lastIndicatorButtonIndex == 0) {
				...
				fromIndex = currentIndex;
				toIndex = fromIndex + lastIndicatorButtonIndex;
			} else if (currentIndex < previousIndex && index == lastIndicatorButtonIndex && lastIndicatorButtonIndex != 0
		...
		protected double computeMinWidth(double height) {
			...
			double leftArrowWidth = snapSize(
					Utils.boundedSize(leftArrowButton.prefWidth(-1), leftArrowButton.minWidth(-1), leftArrowButton.maxWidth(-1)));
			...
			double rightNextPageArrowWidth = snapSize(Utils.boundedSize(rightNextPageArrowButton.prefWidth(-1),
					rightNextPageArrowButton.minWidth(-1), rightNextPageArrowButton.maxWidth(-1)));
			double leftNextPageArrowWidth = snapSize(Utils.boundedSize(leftNextPageArrowButton.prefWidth(-1),
					leftNextPageArrowButton.minWidth(-1), leftNextPageArrowButton.maxWidth(-1)));
			...
			return left + leftNextPageArrowWidth + leftArrowWidth + 2 * arrowGap + minButtonSize /*at least one button*/
		...
		protected void layoutChildren() {
			...
			if (isPageInformationVisible()) {
				...
				layoutInArea(pageInformation, pageInformationX, pageInformationY, pageInformationWidth, pageInformationHeight, 0,
						pageInformationHPos, pageInformationVPos);
			}
	...
	private static class StyleableProperties {
		private static final CssMetaData<Pagination, Boolean> ARROWS_VISIBLE = new CssMetaData<Pagination, Boolean>("-fx-arrows-visible",
		...
		private static final CssMetaData<Pagination, Boolean> PAGE_INFORMATION_VISIBLE = new CssMetaData<Pagination, Boolean>(
				"-fx-page-information-visible", BooleanConverter.getInstance(), DEFAULT_PAGE_INFORMATION_VISIBLE) {
		...
		private static final CssMetaData<Pagination, Side> PAGE_INFORMATION_ALIGNMENT = new CssMetaData<Pagination, Side>(
				"-fx-page-information-alignment", new EnumConverter<Side>(Side.class), DEFAULT_PAGE_INFORMATION_ALIGNMENT) {
		...
		private static final CssMetaData<Pagination, Boolean> TOOLTIP_VISIBLE = new CssMetaData<Pagination, Boolean>("-fx-tooltip-visible",
		...
		private static final CssMetaData<Pagination, Number> ARROW_BUTTON_GAP = new CssMetaData<Pagination, Number>("-fx-arrow-button-gap",
