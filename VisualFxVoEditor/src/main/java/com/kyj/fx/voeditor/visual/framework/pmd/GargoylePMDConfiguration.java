 ...
 * <ul>
 * <li>Suppress marker is used in source files to suppress a RuleViolation, defaults to {@link PMD#SUPPRESS_MARKER}. {@link #getSuppressMarker()}</li>
 * <li>The number of threads to create when invoking on multiple files, defaults one thread per available processor. {@link #getThreads()}</li>
 * <li>A ClassLoader to use when loading classes during Rule processing (e.g. during type resolution), defaults to ClassLoader of the Configuration class. {@link #getClassLoader()}</li>
 * <li>A means to configure a ClassLoader using a prepended classpath String, instead of directly setting it programmatically. {@link #prependClasspath(String)}</li>
 * <li>A LanguageVersionDiscoverer instance, which defaults to using the default LanguageVersion of each Language. Means are provided to change the LanguageVersion for each Language.
 ...
 * <li>A minimum priority threshold when loading Rules from RuleSets, defaults to {@link RulePriority#LOW}. {@link #getMinimumPriority()}</li>
 * <li>The character encoding of source files, defaults to the system default as returned by <code>System.getProperty("file.encoding")</code>. {@link #getSourceEncoding()}</li>
 * <li>A comma separated list of input paths to process for source files. This may include files, directories, archives (e.g. ZIP files), etc. {@link #getInputPaths()}</li>
 * <li>A flag which controls, whether {@link RuleSetFactoryCompatibility} filter should be used or not: #isRuleSetFactoryCompatibilityEnabled;
 * </ul>
 * <p>
 * <ul>
 ...
 * <li>An indicator of whether to use File short names in Reports, defaults to <code>false</code>. {@link #isReportShortNames()}</li>
 * <li>The initialization properties to use when creating a Renderer instance. {@link #getReportProperties()}</li>
 * <li>An indicator of whether to show suppressed Rule violations in Reports. {@link #isShowSuppressedViolations()}</li>
 * </ul>
 * <p>
 * The aspects related to special PMD behavior are:
 * <ul>
 * <li>An indicator of whether PMD should log debug information. {@link #isDebug()}</li>
 * <li>An indicator of whether PMD should perform stress testing behaviors, such as randomizing the order of file processing. {@link #isStressTest()}</li>
 * <li>An indicator of whether PMD should log benchmarking information. {@link #isBenchmark()}</li>
 * </ul>
 */
public class GargoylePMDConfiguration extends PMDConfiguration {
	...
	/**
	 * Set the ClassLoader being used by PMD when processing Rules. Setting a value of <code>null</code> will cause the default ClassLoader to be used.
	 *
	 * @param classLoader
	...
	/**
	 * Prepend the specified classpath like string to the current ClassLoader of the configuration. If no ClassLoader is currently configured, the ClassLoader used to load the
	 * {@link GargoylePMDConfiguration} class will be used as the parent ClassLoader of the created ClassLoader.
	 * <p>
	 * If the classpath String looks like a URL to a file (i.e. starts with <code>file://</code>) the file will be read with each line representing an entry on the classpath.
	 *
	 * @param classpath
	 *            The prepended classpath.
	 * @throws IOException
	 *             if the given classpath is invalid (e.g. does not exist)
	 * @see GargoylePMDConfiguration#setClassLoader(ClassLoader)
	 * @see ClasspathClassLoader
	 */
	public void prependClasspath(String classpath) throws IOException {
		if (classLoader == null) {
			classLoader = GargoylePMDConfiguration.class.getClassLoader();
		}
		if (classpath != null) {
	 ...
	 *
	 * @return <code>true</code> if showing suppressed violations, <code>false</code> otherwise.
	 */
	 ...
	 *            <code>true</code> if showing suppressed violations, <code>false</code> otherwise.
	 */
	...
	/**
	 * Return the stress test indicator. If this value is <code>true</code> then PMD will randomize the order of file processing to attempt to shake out bugs.
	 *
	...
	/**
	 * Return the benchmark indicator. If this value is <code>true</code> then PMD will log benchmark information.
	 *
