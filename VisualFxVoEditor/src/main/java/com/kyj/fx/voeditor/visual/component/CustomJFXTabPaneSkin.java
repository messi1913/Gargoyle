 ...
 */
public class CustomJFXTabPaneSkin extends BehaviorSkinBase<TabPane, TabPaneBehavior> {

	private Color defaultColor = Color.TRANSPARENT, ripplerColor = Color.valueOf("FFFF8D"), selectedTabText = Color.WHITE,
			tempLabelColor = Color.WHITE;
	...
	public CustomJFXTabPaneSkin(TabPane tabPane) {
		...
		for (int i = 0, max = tabContentHolders.size(); i < max; i++) {
			...
			if (tabContentHolder.tab == selectedTab) {
				...
				if (index != i) {
					...
					if (isSelectingTab) {
						new CachedTransition(tabsContainer, new Timeline(new KeyFrame(Duration.millis(1000),
								new KeyValue(tabsContainer.translateXProperty(), -contentWidth * index, Interpolator.EASE_BOTH)))) {
							{
	...
	protected class HeaderContainer extends StackPane {
		...
		public HeaderContainer() {
				...
				protected void layoutChildren() {
					if (isTabsFitHeaderWidth()) {
						...
						if (!removedTabsHeaders.isEmpty()) {
							double offset = 0;
							double w = headerContainer.getWidth() - snapSize(rightControlButton.prefWidth(-1))
									- snapSize(leftControlButton.prefWidth(-1)) - snappedLeftInset() - SPACER;
			...
			selectedTabLine.translateYProperty()
					.bind(Bindings.createDoubleBinding(() -> headersRegion.getHeight() - selectedTabLine.getStrokeWidth() + 1,
							headersRegion.heightProperty(), selectedTabLine.strokeWidthProperty()));
		...
		private void runTimeline(double newTransX, double newWidth) {
			...
			if (transDiff > 0) {
				timeline = new Timeline(
						new KeyFrame(Duration.ZERO, new KeyValue(selectedTabLine.endXProperty(), oldWidth, Interpolator.EASE_BOTH),
						...
						new KeyFrame(Duration.seconds(0.15),
								...
								new KeyValue(selectedTabLine.translateXProperty(), oldTransX + offsetStart, Interpolator.EASE_BOTH)),
						new KeyFrame(Duration.seconds(0.33), new KeyValue(selectedTabLine.endXProperty(), newWidth, Interpolator.EASE_BOTH),
		...
		@Override
		protected double computePrefWidth(double height) {
			return snapSize(headersRegion.prefWidth(height)) + 2 * rightControlButton.prefWidth(height) + snappedLeftInset() + SPACER
	...
	protected class HeaderControl extends StackPane {
		...
		private SVGGlyph leftChevron = new SVGGlyph(0, "CHEVRON_LEFT",
				...
				Color.WHITE);
		private SVGGlyph rightChevron = new SVGGlyph(0, "CHEVRON_RIGHT",
		...
		public HeaderControl(ArrowPosition pos) {
			...
			container.setOnMousePressed(press -> {
				...
				double offset = isLeftArrow ? headerContainer.scrollOffset + headerContainer.headersRegion.getWidth()
						: headerContainer.scrollOffset - headerContainer.headersRegion.getWidth();
