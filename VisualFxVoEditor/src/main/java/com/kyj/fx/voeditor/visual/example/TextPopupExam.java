...
public class TextPopupExam extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		textArea.addEventFilter(KeyEvent.KEY_PRESSED, ev -> {
			...
			if (ev.getCode() == KeyCode.V && ev.isControlDown()) {
				...
				for (String str : set) {
					if (str.trim().isEmpty())
						continue;
					sb.append("[").append(idx++).append("]").append(str).append("\n");
				}
		...
		// // UnaryOperator<Point2D> unaryOperator =
		// textArea.popupAnchorAdjustmentProperty().get();
		// // unaryOperator.
		// TableOpenResourceView tableOpenResourceView = new
		// TableOpenResourceView(() -> {
		// try {
		// return DbUtil.getConnection();
		// } catch (Exception e) {
		// e.printStackTrace();
		...
		// /* (non-Javadoc)
		// * @see
		// com.kyj.fx.voeditor.visual.component.popup.TableOpenResourceView#close()
		// */
		// @Override
		// public void close() {
		//
		...
		//
		// Paragraph<Collection<String>> currentParagraphRange =
		// currentParagraphRange(textArea);
		// IndexRange styleRangeAtPosition =
		// currentParagraphRange.getStyleRangeAtPosition(markTextColumnIndex);
		// IndexRange normalize = IndexRange.normalize(markTextColumnIndex,
		// styleRangeAtPosition.getEnd());
		...
		// };
		//
		// textArea.setPopupWindow(new
		// PopOver(tableOpenResourceView.getView()));
	...
	// }
	//
	// private Paragraph<Collection<String>> currentParagraphRange(CodeArea
	// textArea) {
