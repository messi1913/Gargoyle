 ...
 *
 */
public class HtmlBaseDiffAppController {

	private static Logger LOGGER = LoggerFactory.getLogger(HtmlBaseDiffAppController.class);
	 ...
	 */
	@FXML
	private WebView wvOriginal;// ListView<ChunkWrapper> lvOrinal;

	/**
	 *
	 */
	@FXML
	private WebView wvRevice;// ListView<ChunkWrapper> lvRevice;
	...
	public HtmlBaseDiffAppController() {
	...
	public void setDiffFile(String ordinalSource, String reviceSource) throws Exception {
	...
	void clear() {
		...
		this.wvOriginal.getEngine().loadContent("");
		this.wvRevice.getEngine().loadContent("");
			...
			if (empty) {
				...
				if (REVICED.equals(name)) {
					...
					if (delta != null) {
						...
						setTooltip(new Tooltip(String.format("position[%d] \n%s -> %s", revised.getPosition() + 1, revised.getLines()
								.toString(), original.getLines().toString())));
					}
					// [end] Tooltip
					...
					if (delta != null) {
						Chunk original = delta.getOriginal();
						Chunk revised = delta.getRevised();
						setTooltip(new Tooltip(String.format("position[%d] \n%s -> %s", revised.getPosition() + 1, original.getLines()
								.toString(), revised.getLines().toString())));
					}
					// [end] Tooltip
				}
				setGraphic(graphics);
