...
class TF_IDF2 {
	...
	public TF_IDF2(URLModel[] docs) {
		...
		int nextIdx = 0;
		for (URLModel doc : docs) {
			// 공백분리를 정규식으로 바꿈. - 공백이 분석에 영향을 끼치는듯보이므로..
			for (String word : splitContent(doc)) {
		...
		for (int docIdx = 0; docIdx < docs.length; docIdx++) {
			URLModel doc = docs[docIdx];
			String[] split = splitContent(doc);
			String[] words = split;
		...
		for (int docIdx = 0; docIdx < docs.length; docIdx++) {
//			String doc = docs[docIdx].getContent();
			for (String word : splitContent(docs[docIdx])) {
				if (TextFilter.isFiltering(word))
					continue;
	...
	private String[] splitContent(URLModel doc) {
		return doc.getContent().split("\\s+");
	}
