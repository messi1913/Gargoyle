...
/***************************
 * 
 * 데이터베이스에 save하기 위한 렌더러 처리.
 * 
 * Driver 라이브러리가 반드시 참조되어 있어야함.
 * @author KYJ
 *
 ***************************/
public class DatabaseXmlRenderer extends AbstractIncrementingRenderer {

	public static final String NAME = "dbXml";

	public static final StringProperty ENCODING = new StringProperty("encoding", "XML encoding format, defaults to UTF-8.", "UTF-8", 0);
	
	public static final StringProperty DRIVER = new StringProperty("driver", "Database Driver class", "", 1);
	public static final StringProperty URL = new StringProperty("url", "Database URL", "", 2);
	public static final StringProperty ID = new StringProperty("id", "Database Id", "", 3);
	public static final StringProperty PASSWORD = new StringProperty("password", "Database password", "", 4);
	
	private boolean useUTF8 = false;

	public DatabaseXmlRenderer() {
		super(NAME, "Database save XML format.");
		definePropertyDescriptor(ENCODING);
		definePropertyDescriptor(DRIVER);
		definePropertyDescriptor(URL);
		definePropertyDescriptor(ID);
		definePropertyDescriptor(PASSWORD);
		
	}

	public DatabaseXmlRenderer(String encoding) {
		this();
